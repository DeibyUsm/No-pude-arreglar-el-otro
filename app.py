...
def programa1_2():
    ...
    with open("1_inventario.txt", "r") as f:
        for line in f:
            ...
            if len(partes) == 2:
                nombre_producto, cantidad = partes
                productos.append({'nombre': nombre_producto, 'cantidad': int(cantidad)})
    ...
    if request.method == 'POST':
        if 'agregar' in request.form:
            ...
            cantidades = {producto['nombre']: int(request.form.get(f'cantidad_{producto["nombre"].replace(" ", "_")}', 0)) for producto in productos}
            productos_no_disponibles = []
            hay_compra = any(cantidad > 0 for cantidad in cantidades.values())  # Verificar si hay al menos un producto a comprar

            if not hay_compra:
                mensaje = "Debes seleccionar al menos un producto para comprar."
            else:
                    ...
                    if cantidad > 0:
                        ...
                        for producto in productos:
                            if producto['nombre'] == nombre and producto['cantidad'] < cantidad:
                                productos_no_disponibles.append(nombre)
                                break

                if productos_no_disponibles:
                    mensaje = f"La cantidad de productos no están disponibles para: {', '.join(productos_no_disponibles)}. Por favor, elige otros productos."
                else:
                    for nombre, cantidad in cantidades.items():
                        if cantidad > 0:
                            # Actualizar inventario según la cantidad comprada
                            for _ in range(cantidad):
                                actualizar_inventario(nombre)
                    mensaje = "Compra realizada con éxito."

            return render_template('programa1_2.html', accion='comprar', productos=productos, mensaje=mensaje)

    return render_template('programa1_2.html', accion=request.args.get('accion'), productos=productos, mensaje=mensaje)
